...
import { startSingleJob, stopSingleBackgroundJob } from "@app/utils/job";
import {
  appTimeZone,
  ...
  Query: {
    ...
    async autoRefresh(
      ...
      if (refresh) {
        ...
        stopSingleBackgroundJob(`${req.currentUser?.username}`);
      }
      return {
        refresh,
      };
  ...
  Mutation: {
    ...
    async toggleMonitor(
      ...
      const hasActiveMonitors: boolean = some(
        results,
        (monitor: IMonitorDocument) => monitor.active === true
      );
      /*
